...
public class RadarHighlighter extends PieRadarHighlighter<RadarChart> {
    ...
    public List<Highlight> getHighlightsAtIndex(int index) {
        ...
        for (int i = 0; i < ((RadarData) ((RadarChart) this.mChart).getData()).getDataSetCount(); i++) {
            IDataSet dataSetByIndex = ((RadarData) ((RadarChart) this.mChart).getData()).getDataSetByIndex(i);
            Entry entry = dataSetByIndex.getEntryForIndex(index);
            ...
            this.mHighlightBuffer.add(new Highlight((float) index, entry.getY(), pOut.f75x, pOut.f76y, i, dataSetByIndex.getAxisDependency()));
        }
