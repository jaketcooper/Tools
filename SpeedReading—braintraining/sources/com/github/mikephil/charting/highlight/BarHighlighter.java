...
public class BarHighlighter extends ChartHighlighter<BarDataProvider> {
    ...
    public Highlight getHighlight(float x, float y) {
        ...
        if (set.isStacked()) {
            return getStackedHighlight(high, set, (float) pos.f73x, (float) pos.f74y);
        }
    ...
    public Highlight getStackedHighlight(Highlight high, IBarDataSet set, float xVal, float yVal) {
        ...
        MPPointD pixels = ((BarDataProvider) this.mChart).getTransformer(set.getAxisDependency()).getPixelForValues(high.getX(), ranges[stackIndex].f64to);
        Highlight stackedHigh = new Highlight(entry.getX(), entry.getY(), (float) pixels.f73x, (float) pixels.f74y, high.getDataSetIndex(), stackIndex, high.getAxis());
