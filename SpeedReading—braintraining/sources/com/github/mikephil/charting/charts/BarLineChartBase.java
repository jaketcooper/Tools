...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends IBarLineScatterCandleBubbleDataSet<? extends Entry>>> extends Chart<T> implements BarLineScatterCandleBubbleDataProvider {
    ...
    public void init() {
        ...
        this.mGridBackgroundPaint.setColor(Color.rgb(HebrewProber.NORMAL_NUN, HebrewProber.NORMAL_NUN, HebrewProber.NORMAL_NUN));
    ...
    public void zoomIn() {
        ...
        this.mViewPortHandler.zoomIn(center.f75x, -center.f76y, this.mZoomMatrixBuffer);
    ...
    public void zoomOut() {
        MPPointF center = this.mViewPortHandler.getContentCenter();
        this.mViewPortHandler.zoomOut(center.f75x, -center.f76y, this.mZoomMatrixBuffer);
        this.mViewPortHandler.refresh(this.mZoomMatrixBuffer, this, false);
        MPPointF.recycleInstance(center);
        calculateOffsets();
        postInvalidate();
    ...
    public void zoomToCenter(float scaleX, float scaleY) {
        ...
        this.mViewPortHandler.zoom(scaleX, scaleY, center.f75x, -center.f76y, save);
    ...
    public void zoomAndCenterAnimated(float scaleX, float scaleY, float xValue, float yValue, YAxis.AxisDependency axis, long duration) {
        if (Build.VERSION.SDK_INT >= 11) {
            ...
            addViewportJob(AnimatedZoomJob.getInstance(this.mViewPortHandler, this, getTransformer(axis), getAxis(axis), this.mXAxis.mAxisRange, scaleX, scaleY, this.mViewPortHandler.getScaleX(), this.mViewPortHandler.getScaleY(), xValue, yValue, (float) origin.f73x, (float) origin.f74y, duration));
    ...
    public void moveViewToAnimated(float xValue, float yValue, YAxis.AxisDependency axis, long duration) {
        if (Build.VERSION.SDK_INT >= 11) {
            ...
            float f = xValue;
            addViewportJob(AnimatedMoveViewJob.getInstance(this.mViewPortHandler, f, yValue + ((getAxisRange(axis) / this.mViewPortHandler.getScaleY()) / 2.0f), getTransformer(axis), this, (float) bounds.f73x, (float) bounds.f74y, duration));
    ...
    public void centerViewToAnimated(float xValue, float yValue, YAxis.AxisDependency axis, long duration) {
        if (Build.VERSION.SDK_INT >= 11) {
            MPPointD bounds = getValuesByTouchPoint(this.mViewPortHandler.contentLeft(), this.mViewPortHandler.contentTop(), axis);
            float yInView = getAxisRange(axis) / this.mViewPortHandler.getScaleY();
            addViewportJob(AnimatedMoveViewJob.getInstance(this.mViewPortHandler, xValue - ((getXAxis().mAxisRange / this.mViewPortHandler.getScaleX()) / 2.0f), (yInView / 2.0f) + yValue, getTransformer(axis), this, (float) bounds.f73x, (float) bounds.f74y, duration));
            MPPointD.recycleInstance(bounds);
            return;
        }
    ...
    public float getLowestVisibleX() {
        ...
        return (float) Math.max((double) this.mXAxis.mAxisMinimum, this.posForGetLowestVisibleX.f73x);
    }
    ...
    public float getHighestVisibleX() {
        ...
        return (float) Math.min((double) this.mXAxis.mAxisMaximum, this.posForGetHighestVisibleX.f73x);
    }
