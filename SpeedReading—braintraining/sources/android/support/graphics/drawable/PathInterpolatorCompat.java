...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (((double) Math.abs(this.f15mX[0])) > 1.0E-5d || ((double) Math.abs(this.f16mY[0])) > 1.0E-5d || ((double) Math.abs(this.f15mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f16mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f15mX[0] + "," + this.f16mY[0] + " end:" + this.f15mX[numPoints - 1] + "," + this.f16mY[numPoints - 1]);
        }
        float prevX = 0.0f;
        int i2 = 0;
    ...
    public float getInterpolation(float t) {
        ...
        return ((this.f16mY[endIndex] - startY) * ((t - this.f15mX[startIndex]) / xRange)) + startY;
    }
}
