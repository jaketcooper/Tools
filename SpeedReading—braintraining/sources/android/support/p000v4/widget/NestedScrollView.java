...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView {
        ...
        if (canScroll()) {
            ...
            if (event.getAction() == 0) {
                switch (event.getKeyCode()) {
                    ...
                    case 20:
                        if (event.isAltPressed()) {
                            handled = fullScroll(SJISContextAnalysis.HIRAGANA_HIGHBYTE);
                            break;
                        } else {
                            handled = arrowScroll(SJISContextAnalysis.HIRAGANA_HIGHBYTE);
                            break;
                        }
                    case 62:
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, SJISContextAnalysis.HIRAGANA_HIGHBYTE);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(SJISContextAnalysis.HIRAGANA_HIGHBYTE)) {
                return false;
            }
            return true;
        }
    ...
    public boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = SJISContextAnalysis.HIRAGANA_HIGHBYTE;
        } else if (direction == 1) {
            direction = 33;
        }
